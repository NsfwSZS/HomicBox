...
function ulx.ban( calling_ply, target_ply, minutes, reason )
	...
	local form = {
		...
		["embeds"] = {{
			["title"] = "Хмм, походу у нас 200 | Игрок забанен...",
			["description"] = (IsValid(calling_ply) and calling_ply:GetName() or "Консоль").." забанил "..target_ply:Nick().." на "..ULib.secondsToStringTime( minutes * 60 ),
			["color"] = 10038562
		}}
	}
...
function ulx.banid( calling_ply, steamid, minutes, reason )
	...
	local form = {
		...
		["embeds"] = {{
			["title"] = "Хмм, походу у нас 200 | Игрок забанен...",
			["description"] = (IsValid(calling_ply) and calling_ply:GetName() or "Консоль").." забанил "..displayid.." на "..ULib.secondsToStringTime( minutes * 60 ),
			["color"] = 10038562
		}}
	}
