if SERVER then
	...
	-- Set us up the texture
		...
		PrintWeaponInfo(self, x + wide + 20, y + tall * 0.95, alpha )
	end
		...
		if not self:GetFists() then
			...
			if Tr.Hit then
				if self:CanPickup(Tr.Entity) then
					...
					col.a = 255 * Size * 2
					--draw_DrawText( Tr.Entity:IsPlayer() and Tr.Entity:Name() or Tr.Entity:GetNWString("Nickname") or "", "HomigradFontLarge", Tr.HitPos:ToScreen().x, Tr.HitPos:ToScreen().y - 30, col, TEXT_ALIGN_CENTER )
...
function SWEP:Holster()
	...
	if CLIENT and self.Owner == LocalPlayer() then
		timer.Simple(.1,function()
			self.Owner:ManipulateBoneAngles(self.Owner:LookupBone("ValveBiped.Bip01_R_Clavicle"),Angle(0,0,0),false)
			self.Owner:ManipulateBoneAngles(self.Owner:LookupBone("ValveBiped.Bip01_L_Clavicle"),Angle(0,0,0),false)
		end)
	end
	return true
end
...
function SWEP:Think()
	...
	if CLIENT then
		self.Owner:ManipulateBoneAngles(self.Owner:LookupBone("ValveBiped.Bip01_R_Clavicle"),Angle(25,0,55)*self.Anim,false)
		self.Owner:ManipulateBoneAngles(self.Owner:LookupBone("ValveBiped.Bip01_L_Clavicle"),Angle(-25,15,-55)*self.Anim,false)
	end
