	...
	for armorInfo,armorData in pairs(armors) do
		...
		if dmginfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT) then
			if (slots.mouthnose or slots.head) then
				sound.Emit(ent,"physics/metal/metal_solid_impact_bullet"..math.random(1,4)..".wav",90)
			...
			then
				...
				sound.Emit(ent,"physics/plastic/plastic_box_impact_bullet5.wav",90,math.random(90,110))
			end
		end

		if dur >= 0.25 then
			armorDur = (armorData.dur/600) * dur
			--dur = math.max(dur - 0.5,0)

			armorMul = math.max(1 - armorDur,0.1)
