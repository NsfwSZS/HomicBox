hook.Add("HomigradDamage","Organs",function(ply,hitgroup,dmginfo,rag,armorMul,armorDur,haveHelmet)
    ...
    if dmginfo:GetDamage() >= 40 and dmginfo:GetAttacker()!="prop_ragdoll" or (dmginfo:GetDamageType() == DMG_CRUSH and dmginfo:GetDamage() >= 25 and ent:GetVelocity():Length() > 1600 and dmginfo:GetAttacker()!="prop_ragdoll") then
        ...
        if brokenLeftArm then
            ...
            if ply.msgLeftArm < CurTime() then
                ...
                ent:EmitSound("NPC_Barnacle.BreakNeck",70,65,0.1,CHAN_ITEM)
            end
        end
    ...
    if not rag or (rag and not dmginfo:IsDamageType(DMG_CRUSH)) then
        ...
        if huy then --ply:ChatPrint("You were hit in the heart.")
            if ply.Organs['heart']~=0 and !dmginfo:IsDamageType(DMG_CLUB) then
                ...
                if ply.Organs['heart']==0 then ply:ChatPrint("Ты чувствуешь острую боль в сердце.") end
            end
        end
...
hook.Add("HomigradDamage","BurnDamage",function(ply,hitgroup,dmginfo) 
    if dmginfo:IsDamageType( DMG_FALL ) and !ply.fake then
