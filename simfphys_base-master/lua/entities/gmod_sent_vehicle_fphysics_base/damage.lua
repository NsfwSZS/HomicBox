function ENT:ApplyDamage( damage, type )
	if type == DMG_BLAST then 
		self:HurtPlayers( damage/(5*(self:GetPhysicsObject():GetMass()/800)) )
		damage = damage * 10
	end
...
function ENT:HurtPlayers( damage, damagetype )
	...
	if IsValid( Driver ) then
		if self.RemoteDriver ~= Driver then
			...
			dmginfo:SetDamageType( damagetype or DMG_DIRECT )
	...
	if ( data.Speed > 60 and data.DeltaTime > 0.02 ) then
		...
		local pos = data.HitPos
			...
			if (data.Speed > 100) then
				--print("He's dead")
				local hitent = data.HitEntity:IsPlayer() or data.HitEntity:IsRagdoll()
				if not hitent then
					self:TakeDamage( (data.Speed / 14) * simfphys.DamageMul, Entity(0), Entity(0) )
					self:HurtPlayers( data.Speed / 30, DMG_DIRECT )
				end
			end
		end
end
