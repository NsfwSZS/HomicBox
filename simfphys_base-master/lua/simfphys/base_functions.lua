	...
	function simfphys.SpawnVehicle( Player, Pos, Ang, Model, Class, VName, VTable, bNoOwner )
		...
		if VTable.Members then
			...
			function Ent:PhysicsCollide( data, physobj )
				...
				--print(data.DeltaTime)
				
				if ( data.Speed > 60 and data.DeltaTime > 0.02 ) then
					...
					local pos = data.HitPos
						...
						if (data.Speed > 200) then
							--print("He's dead")
							local hitent = data.HitEntity:IsWorld()
							if hitent then
								self:TakeDamage( (data.Speed / 14) * simfphys.DamageMul, Entity(0), Entity(0) )
								self:HurtPlayers( data.Speed / 35, DMG_DIRECT )
								
								Spark( pos , data.HitNormal , (data.Speed<400 and "MetalVehicle.ImpactSoft") or (data.Speed>=400 and "MetalVehicle.ImpactHard") )
							elseif data.HitEntity:GetPhysicsObject() then
								local hitphysic = data.HitEntity:GetPhysicsObject()
								local carphysic = self:GetPhysicsObject()

								local takedamage = hitphysic:GetMass()/carphysic:GetMass()
								if (not hitphysic:IsMotionEnabled()) and data.HitEntity:Health()==0 then
									
									takedamage = 1
								end

								self:TakeDamage( (data.Speed / 14*takedamage) * simfphys.DamageMul, Entity(0), Entity(0) )
								self:HurtPlayers( data.Speed / 35*takedamage, DMG_DIRECT )
