	...
	for k, playa in pairs(player.GetAll()) do
		if playa ~= myself then
			...
			function Panel:Paint(w, h)
				...
				draw.SimpleText((playa:IsValid() and playa:Nick()) or "ВЫШЕЛ", "DermaDefault", 5, 3, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
			end
...
net.Receive("JMod_EZtoolbox", function()
	...
	StandardSelectionMenu('crafting', "Набор сборщика", Buildables, Kit, function(name, info, ply, ent) -- enable func
...
net.Receive("JMod_EZworkbench", function()
	...
	StandardSelectionMenu('crafting', "Верстак", Buildables, Bench, function(name, info, ply, ent) -- enable func
...
net.Receive("JMod_EZtimeBomb", function()
	...
	frame:SetTitle("Бомба с таймером")
	frame:SetDraggable(true)
	frame:Center()
	frame:MakePopup()
...
net.Receive("JMod_ModifyMachine", function()
	...
	frame:SetTitle("Модификация механизма")
	frame:SetDraggable(true)
	frame:Center()
	frame:MakePopup()
...
net.Receive("JMod_EZradio", function()
	...
	if isMessage then
		...
		local tbl = {radio:GetColor(), "Авиа транзит", Color(255, 255, 255), ": ", msg}

		if parrot then
			tbl = {Color(200, 200, 200), "(Скрыто) ", LocalPlayer(), Color(255, 255, 255), ": ", Color(200, 200, 200), msg}
		end
	...
	StandardSelectionMenu('selecting', "Радио", Orderables, Radio, function(name, info, ply, ent)
		-- enable func
...
local function CreateArmorSlotButton(parent, slot, x, y)
	...
	if ItemID then
		local str = "Долговечность: " .. math.Round(ItemData.dur, 1) .. "/" .. ItemInfo.dur
		...
		Buttalony:SetTooltip("Слот пуст")
	end
...
net.Receive("JMod_Inventory", function()
	...
	motherFrame:SetDraggable(true)
	motherFrame:ShowCloseButton(true)
	motherFrame:SetTitle("Инвентарь | Текущий вес брони: " .. weight .. "кг.")
