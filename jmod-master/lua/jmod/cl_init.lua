		...
		if ply:Alive() and ply.EZarmor and ply.EZarmor.effects then
			if ply.EZarmor.effects.nightVision or ply.EZarmor.effects.nightVisionWP then
				...
				if not IsValid(ply.EZNVGlamp) then
					...
					ply.EZNVGlamp:SetBrightness(.015)
				else
					local Dir = ply:GetAimVector()
					local Ang = Dir:Angle()
					ply.EZNVGlamp:SetPos(ply:EyePos() + Dir * 40)
					ply.EZNVGlamp:SetAngles(Ang)
					ply.EZNVGlamp:SetConstantAttenuation(.6)
					local FoV = ply:GetFOV()
					ply.EZNVGlamp:SetFOV(FoV)
					ply.EZNVGlamp:SetFarZ(250000 / FoV)
...
hook.Add("PostDrawOpaqueRenderables", "JMOD_POSTOPAQUERENDERABLES", function()
	...
	if ply:Alive() and ply.EZarmor and ply.EZarmor.effects and ply.EZarmor.effects.thermalVision then
