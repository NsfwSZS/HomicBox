...
function JMod.LocationalDmgHandling(ply, hitgroup, dmg)
	...
	if AmmoTypeID then
		...
		if AmmoName then
			local AmmoInfo = dmg:GetDamageForce():Length()
...
hook.Add("PlayerFootstep", "JMOD_PlayerFootstep", function(ply, pos, foot, snd, vol, filter)
	if ply.EZarmor then
		...
		if ply.EZarmor.totalWeight >= 20 then
			ply:EmitSound("snd_jack_gear" .. tostring(math.random(1, 6)) .. ".wav", 55, math.random(70, 130),0.5)
		end
	end
end)
...
net.Receive("JMod_Inventory", function(ln, ply)
	...
	elseif ActionType == 3 then
		...
		if RepairStatus == 0 then
			ply:PrintMessage(HUD_PRINTCENTER, "предмет не починить")
		elseif RepairStatus == 1 then
			...
			ply:PrintMessage(HUD_PRINTCENTER, "нету матерьялов для починки, нужно " .. mats)
		elseif RepairStatus == 2 then
			ply:PrintMessage(HUD_PRINTCENTER, "предмет починен")
	...
	elseif ActionType == 4 then
		...
		if RechargeStatus == 0 then
			ply:PrintMessage(HUD_PRINTCENTER, "предмет не может быть перезаряжен")
		elseif RechargeStatus == 1 then
			...
			ply:PrintMessage(HUD_PRINTCENTER, "нехватает матерьялов для перезарядки, нужно " .. mats)
		elseif RechargeStatus == 2 then
			ply:PrintMessage(HUD_PRINTCENTER, "предмет перезаряжен")
