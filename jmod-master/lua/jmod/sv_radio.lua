...
local NotifyAllMsgs = {
	["normal"] = {
		...
		["good drop"] = "хорошая доставка, посылка доставлена, возвращение на базу...",
		["drop failed"] = "сброс не выполнен, пилот не смог определить подходящую позицию сброса по указанным координатам. Самолет находится в режиме RTB",
		["drop soon"] = "имейте в виду, самолет на месте, падение посылки на место вызова неизбежно",
		["ready"] = "внимание, этот аванпост теперь готов выполнять миссии по доставке"
	},
	["bff"] = {
	...
	for stationID, station in pairs(JMod.EZ_RADIO_STATIONS) do
		...
		if station.restrictedPackageDelivering then
			if station.restrictedPackageDeliveryTime < Time then
				...
				NotifyAllRadios(stationID, "Внимание, этот аванпост получил специальную партию " .. station.restrictedPackageDelivering .. " из регионального штаба", true)
		...
		for id, data in pairs(ply.EZarmor.items) do
			...
			if Info.eff and Info.eff.teamComms then
				...
				if data.chrg.power <= Info.chrg.power * .25 then
					JMod.EZarmorWarning(ply, "Электрический заряд брони почти исчерпан!")
				end
			end
		end
...
function JMod.AddNewRadioOutpost(teamID)
	...
	for k, ply in pairs(player.GetAll()) do
		ply:PrintMessage(HUD_PRINTTALK, "Команда " .. team.GetName(tonumber(teamID)) .. " получил радиостанцию.")
	end
end
...
function JMod.RemoveRadioOutPost(teamID)
	...
	for k, ply in pairs(player.GetAll()) do
		for k, ply in pairs(player.GetAll()) do
			ply:PrintMessage(HUD_PRINTTALK, "Команда " .. team.GetName(tonumber(teamID)) .. " проебала радиостанцию.")
		end
	end
end
...
local function StartDelivery(pkg, transceiver, id, bff, ply)
	...
	if bff then return "эй, ХОРОШ СПАМИТЬ, чувак, мы присылаем " .. GetArticle(pkg) .. " " .. pkg .. " коробку прямо к " .. math.Round(Pos.x) .. " " .. math.Round(Pos.y) .. " " .. math.Round(Pos.z) .. " примерно через " .. DeliveryTime .. " сек." end

	return "Вас понял, отправка " .. GetArticle(pkg) .. " " .. pkg .. " пакет по координатам " .. math.Round(Pos.x) .. ", " .. math.Round(Pos.z) .. "; расчетное время прибытия " .. DeliveryTime .. " сек."
end
			...
			if table.HasValue(Station.restrictedPackageStock, pkg) then
				...
				if Station.restrictedPackageDelivering then
					if bff then
						...
						return "отрицательный ответ на этот запрос, у нас ничего этого нет на складе, и в настоящее время штаб-квартира доставляет еще одну спец парти..."
					end
				else
					...
					if bff then
						...
						return "Приём, у нас ничего этого нет в наличии, но мы заказали это в региональной штаб-квартире, оно будет на складе через " .. GetTimeString(DeliveryTime)
					end
				end
			end
		else
...
function JMod.EZradioStatus(transceiver, id, ply, bff)
	...
	if Station.state == JMod.EZ_STATION_STATE_DELIVERING then
		Msg = "этот аванпост в настоящее время доставляет посылку"

		if bff then
			Msg = "эй, мы сейчас достовляем кое-что для кого-то еще"
		end
	elseif Station.state == JMod.EZ_STATION_STATE_BUSY then
		Msg = "эта застава в настоящее время готовится к поставкам"

		if bff then
			Msg = "эй, братан, мы сейчас очень заняты, просто подожди"
		end
	elseif Station.state == JMod.EZ_STATION_STATE_READY then
		Msg = "этот аванпост готов принимать миссии по доставке"

		if bff then
			Msg = "ВСЕ, ЧТО ВАМ НУЖНО, МЫ ПОЛУЧИЛИ"
		end
	end
	...
	if Station.restrictedPackageDelivering then
		Msg = Msg .. ", и имеет специальную доставку " .. Station.restrictedPackageDelivering .. " прибывший из регионального штаба в г. " .. GetTimeString(Station.restrictedPackageDeliveryTime - Time)
	end
